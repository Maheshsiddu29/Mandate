...
Phase 5 applies the same method to the advisory layer: scenarios describe
inputs, `@mandate/jev` computes results, and the committed report is
regenerated by a test. Methodology and the safety-versus-quality distinction
are in [jev-evaluation.md](jev-evaluation.md); the report is
`corpus/jev-evaluation-v1`. Jev is simulated by stubs there, so a large
adversarial run costs no external requests and the report carries no latency
figure.
