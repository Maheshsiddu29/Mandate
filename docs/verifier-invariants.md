...
> **Status: Phase 5R.3, implemented.** Everything below is enforced by
...
| V-2 | **Total over parsed, plain values.** Every value `JSON.parse` can produce — plus the kernel's own value types — yields a receipt; nothing throws to signal a financial outcome. | `verify` takes `unknown`, first normalizes the top-level request boundary, then parses every field independently. Null, primitives, arrays, empty objects, malformed nested values, 400 generated junk inputs, 200 JSON-round-tripped payloads, a 16×16 junk matrix and 200 partially-valid inputs all produce rejecting receipts. **Corrected in Phase 5R:** the claim was false at the encoding boundary — an unbounded collection made `trustedStateDigest` throw a `u16` range assertion, so a refusal produced no receipt at all. Every collection the encoders count is now bounded by its parser, tested below, at and above each limit, and the digest step is guarded so an internal invariant failure rejects with `VERIFIER_INTERNAL_ERROR` rather than escaping. **The kernel outer request was completed in Phase 5R.2; the cross-package exported-boundary claim is completed in Phase 5R.3. Scoped in Phase 5R.1** — see V-9 and [the public boundary inventory](public-trust-boundaries.md) | TEST, PROPERTY |
